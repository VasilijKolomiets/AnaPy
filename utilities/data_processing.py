...
def addresses_processing(path_ro_xlsx: Path, sheet_name='АП'):
    ...
    df_addrs.квартира.fillna('', inplace=True)

    df_addrs.дом.replace('nan', np.nan, inplace=True)
    df_addrs.дом.fillna('', inplace=True)

    df_addrs.zipCode.replace('nan', np.nan, inplace=True)
    df_addrs.zipCode.fillna('', inplace=True)

    df_addrs.телефон.replace('nan', np.nan, inplace=True)
    df_addrs.телефон.fillna('', inplace=True)
...
def receive_addressIDs_by_api(postman: ua_posts_api.Postman, df_addrs):
    ...
    # =============================================================================
    new_fields = ['CityID', 'AddressID', 'parcelID',
                  'itemsID_list',      # TODO: for what?
                  'post_city_name',    # for post service variant city name
                  'post_street_name',  # for post service variant street name
                  ]
    df_addrs.loc[:, new_fields] = ''

    for index, row in df_addrs.iterrows():

        print(index, "\n", row, "\n")
        if not row.zipCode:  # CityID by ukr_name
            code, response_ = postman.search.by_ukr_name_city_id(city_descr=row.город)
            city_ids = postman._city_by_ukrname_wise_pick(
                response_,
                city_name=row.город,
            )
        else:  # CityID by zipCode
            code, response_ = postman.search.by_zip_city_id(zipCode=row.zipCode)
            city_ids = postman._city_by_ZIP_wise_pick(
                response_,
                city_name=row.город,
                zipCode=row.zipCode
            )
        ...
        if type(city_ids) == list:   # TODO: not unique city name ...
            df_addrs.at[index, 'CityID'] = city_ids
            continue
        elif type(city_ids) != tuple:
            assert False, "wrong 'city_ids' type. have to be or list or tuple "

        df_addrs.at[index, 'CityID'], df_addrs.at[index, 'post_city_name'] = city_ids
        # AddressID by CityID
        code, response = postman.search.address_id(city_id=city_ids[0], streetname=row.улица)
        if len(response) == 1:  # distinct answered street - is very good
            print(row.город, row.zipCode, 'resp code=', code, response[0]['addressID'], "--Ok--")
            df_addrs.at[index, 'AddressID'] = response[0]['addressID']
            df_addrs.at[index, 'post_street_name'] = response[0]["addressDescr"]["descrUA"]
        else:
...
def _items_processing(path_to_xlsx: Path, sheet_name='Код_Прод'):
    """Proces the items table.

    """
    df_items = pd.read_excel(path_to_xlsx, sheet_name=sheet_name,
                             header=0,
...
def items_processing(path_to_xlsx: Path, sheet_name='Код_Прод'):
    """Proces the items table.

    fields in 1st row xlsx file (like in 'GOOD_Пумб.xlsx'):

    'код_поставки', 'item_id', 'наименование_изделия',
    'weight_1', 'value_1',
    'length', 'width',
    'height_x_100',
    """
    df_items = pd.read_excel(
        path_to_xlsx, sheet_name=sheet_name,
        header=0,
        usecols=list(range(8)),  # TODO: 8 or 9 ?
        dtype=dict(
            weight_1=float,
            value_1=float,
            length=float,
    ...
    return (df_items.код_поставки.unique()[0], df_items)


def _apointments_processing(path_ro_xlsx: Path, sheet_name='ALL_Адрес'):
...
def apointments_processing(path_ro_xlsx: Path, sheet_name='ALL_Адрес'):
    ...
    df_apointments = pd.read_excel(path_ro_xlsx, sheet_name=sheet_name,
                                   ...
                                   # skiprows=0,
                                   # !!! 'receivers_id' was 'addr_id' in _apointments_processing
                                   names=['delivery_id', 'receivers_id', 'item_id_in_delivery',
                                          'parcels', 'items_total',
                                          # 'items_in_parcel',  # tricked emptycolumn
                                          ],
                                   dtype=dict(
                                       delivery_id=int,
                                       receivers_id=int,
                                       item_id_in_delivery=int,
                                       parcels=int,
                                       items_total=int,
                                       # items_in_parcel=float,
                                   ),
...
def _create_parcels_by_api(postman: ua_posts_api.Postman, df_addrs, df_common):
...
def _get_stickers_by_api(postman: ua_posts_api.Postman, df_addr,
...
if __name__ == '__main__':
    post_cervice_params = credentials['Meest']
    postman = ua_posts_api.Postman(post_cervice_params)
    ...
    df_items = _items_processing(path_ro_xlsx)
    df_items.to_excel('items_' + client_name + str(time.time()).replace('.', '_')
                      + '.xlsx', engine='xlsxwriter')

    df_apointments = _apointments_processing(path_ro_xlsx)
    ...
    df_addrs = _create_parcels_by_api(postman, df_addrs, df_common)
    ...
    _get_stickers_by_api(postman, df_common)
